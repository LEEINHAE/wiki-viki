...
export function slugify(value) {
	return value.trim().replace(/[\s_]+/g, '-').replace(/[^\p{L}\p{N}:.~-]/gu, '').replace(/-+/g, '-').toLowerCase();
}
...
export async function getDocument(slug) {
	...
	const [redirect] = await sql`SELECT d.*, r.alias_title FROM redirects r JOIN documents d ON d.id = r.document_id WHERE r.alias_slug = ${slug} LIMIT 1`;
	return redirect ? { document: redirect, redirectedFrom: redirect.alias_title } : null;
}

export async function saveDocument({ title, content, editor = 'Editor-01', summary = '', originalSlug }) {
	const sql = db();
...
export async function renderWiki(content) {
	...
	let source = content.replace(/^\[\^([^\]]+)\]:\s*(.+)$/gm, (_, id, note) => { footnotes.set(id, note); return ''; });
	...
	source = source.replace(/\[\^([^\]]+)\]/g, (_, id) => `@@WIKIFOOTNOTE:${slugify(id)}:${id.replace(/[^\p{L}\p{N}_.-]/gu, '')}@@`);
	const renderer = new marked.Renderer();
	let headingIndex = 0;
	renderer.heading = ({ tokens, depth }) => `<h${depth} id="section-${++headingIndex}">${marked.parser(tokens)}</h${depth}>`;
	...
	html = html.replace(/@@WIKIFOOTNOTE:([^:]+):([^@]+)@@/g, '<sup class="footnote-ref"><a href="#fn-$1">[$2]</a></sup>');
	if (footnotes.size) html += `<section class="footnotes"><hr><ol>${[...footnotes].map(([id, note]) => `<li id="fn-${escapeHtml(slugify(id))}">${escapeHtml(note)}</li>`).join('')}</ol></section>`;
	return html;
}

export async function autoLinkDocument(content, suggestions = []) {
	const terms = [...new Set(suggestions.map((term) => term.trim()).filter((term) => term.length > 2))].slice(0, 20);
	if (!terms.length) return content;
	const sql = db();
	const rows = await sql`SELECT title FROM documents WHERE LOWER(title) IN ${sql(terms.map((term) => term.toLowerCase()))}`;
	let linked = content;
	for (const { title } of rows) {
		if (linked.includes(`[[${title}`)) continue;
		const escaped = title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
		linked = linked.replace(new RegExp(`(?<!\\[\\[)\\b${escaped}\\b(?![^[]*\\]\\])`, 'u'), `[[${title}]]`);
	}
	return linked;
}
...
export function buildToc(content) {
	...
	return [...content.matchAll(/^(#{1,6})\s+(.+)$/gm)].map((match, index) => {
		...
		return { level, title: match[2].replace(/[*_`~]/g, ''), number: counters.slice(0, level).filter(Boolean).join('.'), id: `section-${index + 1}` };
	});
}
