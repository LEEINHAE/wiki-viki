...
export const actions = { rollback: async ({ request, params }) => {
	...
	if (!revision) return fail(404, { message: 'Revision not found.' });
	await saveDocument({ title: result.document.title, content: revision.content, editor: 'Operator-A', summary: `Rollback to revision ${id}`, originalSlug: result.document.slug });
