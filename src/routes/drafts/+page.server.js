...
export const actions = {
	update: async ({ request }) => {
		...
		if (!title || !content || !validHandle(editor)) return fail(400, { message: 'Title, content, and a valid anonymous handle are required.' });
	...
	publish: async ({ request }) => {
		...
		if (!draft) return fail(404, { message: 'Draft not found.' });
		...
		if (!regex.passed || !semantic.passed) { await db()`UPDATE drafts SET status='blocked',governance=${db().json({ passed:false,regex,semantic })},updated_at=NOW() WHERE id=${id}`; return fail(400, { message: 'Publish blocked by governance. Review the flagged content.' }); }
		const document = await saveDocument({ title: draft.title, content: draft.content, editor: draft.editor_handle, summary: `Published from draft ${id}`, originalSlug: draft.slug });
